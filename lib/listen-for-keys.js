...
const onKeypress = (stream, cb) => {
	const decoder = new StringDecoder('utf8')
	const onData = (data) => {
		const parsed = parseKeypress(decoder.write(data), stream.encoding)
...
const parseKeypress = (s, enc) => {
	...
	} else if (s.charCodeAt(0) === 8747) {
		key.name = 'left' // alt + B
	} else if (s.charCodeAt(0) === 402) {
		key.name = 'right' // alt + F
	}
...
const parseAction = (key) => {
	...
	if (key.name === 'space')  return 'pause'

	if (key.name === 'right')  return 'scrub-right'
	if (key.name === 'left')   return 'scrub-left'

	// if (key.name === 'up')    return 'volume-up'
	// if (key.name === 'down')  return 'volume-down'

	return false
}

const listen = (p, onRaw, onAction) => {
	const onKey = (key) => {
		const action = parseAction(key)
		if (action === false) onRaw(key)
		else onAction(action)
	}
	const offKeypress = onKeypress(process.stdin, onKey)
...
module.exports = Object.assign(listen)
